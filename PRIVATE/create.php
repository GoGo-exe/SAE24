<?php
   ...
   if(isset($valider)){
      ...
      elseif($pass!=$repass) $erreur="Mots de passe non identiques!";
         ...
         $sel=$pdo->prepare("select id from Client where login=? limit 1");
         $sel->execute(array($login));
         $tab=$sel->fetchAll();
         if(count($tab)>0)
            $erreur="Login existe déjà!";
         else{
            $ins=$pdo->prepare("insert into Client(nom,prenom,login,password) values(?,?,?,?)");
...
<html>
   ...
   <body>
      ...
      <form name="fo" method="post" action="">
         ...
         <input type="submit" name="valider" value="Créer un utilisateur" />
      </form>
   </body>
</html>